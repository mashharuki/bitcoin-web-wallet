...
Insight.prototype.getUnspentUtxos = function(addresses, callback) {
  ...
  // API実行してＨＴＴＰリクエスト
  /*
  this.requestPost('/address/' + addresses.toString() + '/utxo', {
  ...
  */
  ...
  // 以下、HTTPリクエスト自分で作り直し
  request('https://blockstream.info/testnet/api/address/' + addresses.toString() + '/utxo', function (error, response , unspent) {
    // Print the HTML for the Google homepage.
    console.log('https://blockstream.info/testnet/api/address/' + addresses.toString() + '/utxo');
    // 取得結果をJSONに変換する
    var unspentJson = JSON.parse(unspent);

    try {
        // 一つずつ出力
        for (var item in unspentJson) {
            // console.log(item + '番目のUTXOS:', unspentJson[item]);
        }
    } catch (ex) {
        if (ex instanceof bitcore.errors.InvalidArgument) {
            return callback(ex);
    ...
    return callback(null, unspentJson);
  });
};
...
Insight.prototype.broadcast = function(transaction, callback) {
  ...
  */
  ...
  // 以下、HTTPリクエスト自分で作り直し
  request('https://blockstream.info/testnet/api/tx/' + transaction , function (err, res) {
    if (err || res.statusCode !== 200) {
      return callback(err || res);
    }
    return callback(null, res ? res.body.txid : null);
  });
};
